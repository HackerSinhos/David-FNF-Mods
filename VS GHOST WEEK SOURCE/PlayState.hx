...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'phantom':
				dialogue = CoolUtil.coolTextFile(Paths.txt('phantom/phantomDialogue'));
			case 'specter':
				dialogue = CoolUtil.coolTextFile(Paths.txt('specter/specterDialogue'));
			case 'polterghost':
				dialogue = CoolUtil.coolTextFile(Paths.txt('polterghost/polterghostDialogue'));
			case 'paranormal':
				dialogue = CoolUtil.coolTextFile(Paths.txt('paranormal/paranormalDialogue'));
			case 'dinner':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dinner/dinnerDialogue'));
		}

		switch(SONG.song.toLowerCase())
		{
			case 'paranormal' | 'monster' | 'dinner': 
			{
				curStage = 'spooky';
				halloweenLevel = true;
		...
		healthBar.createFilledBar(0xFFD3D3D3, 0xFF66FF33);
		// healthBar
		add(healthBar);
